...
const BarChartExample = () => {
  ...
  if (data.length > 0) {
    maxTotalAmount = Math.max(...data.map(item => parseInt(item.totalAmount, 10)));
  }
  ...
  return (
    <div style={{ width: '100%', height: `${calculatedHeight}px` }}>
      ...
      >
        ...
        <XAxis dataKey="month" />
        <YAxis domain={[0, maxTotalAmount + 1000000]} /> {/* Điều chỉnh domain của trục Y */}
        <Tooltip />
        <Legend />
        <Bar dataKey="totalAmount" fill="#8884d8" barCategoryGap="25%" /> {/* Tăng khoảng cách giữa các cột */}
      </BarChart>
    </div>
  );
};
